"""
train_dqn_v4b_curriculum_rs.py  — v4b: Curriculum scenario sampling + reward shaping.

LINEAGE
-------
Exactly train_dqn_v3_ddqn_per.py (Double DQN + Prioritized Experience Replay)
with two targeted additions, EVAL PROTOCOL UNCHANGED throughout:

  1. Env import -> rl_env_v4 (link-quality-weighted r_prog)          [v4b new]
     Same reward shaping as v4a. Pre-flight check (src/preflight_reward_v4.py)
     found it's nearly inert in this distance-only link model — included here
     per spec (v4b = Curriculum + RS) but expect it to contribute ~nothing,
     same as the v4a RS-only isolation arm confirmed empirically.

  2. Curriculum scenario sampling replacing uniform round-robin        [v4b new]
     OLD: sc = scenario_names[ep % len(scenario_names)]   (deterministic rotation)
     NEW: sc = curriculum_sample(ep, rng)                 (weighted random draw)

     3 tiers by actual dataset PDR (from spec, Section 5/Stage 2):
       Easy:   rwp_sc01(0.837), rwp_sc02(0.802), rwp_sc03(0.737)
       Medium: rwp_sc10(0.594), rwp_sc04(0.466), rwp_sc07(0.462)
       Hard:   rwp_sc15(0.449), rwp_sc12(0.275), rwp_sc08(0.202)

     Piecewise-linear tier-weight schedule (each tier's weight split evenly
     across its 3 scenarios):
       ep    0 - 1500 : (Easy, Medium, Hard) held at (0.80, 0.15, 0.05) — mostly Easy
       ep 1500 - 3000 : linearly blends to  (0.40, 0.40, 0.20) — Medium blended in
       ep 3000 - 5000 : linearly blends to  (0.333, 0.333, 0.333) — Hard blended in,
                        converging to uniform-over-9 by the final episode
     Sampling uses a DEDICATED seeded Generator (np.random.default_rng(seed+500000),
     decorrelated from ep_seed_rng and from PER's global np.random stream) so the
     curriculum draw is reproducible per --seed without perturbing env-seeding or
     PER-sampling reproducibility.

     EVAL PROTOCOL IS UNTOUCHED: EVAL_SCENARIOS (sc03/sc10/sc07), eval seeds,
     eval_packets, GENERALIZATION_SCENARIOS all identical to v3. Only which
     TRAINING scenario gets sampled each episode changes.

v3 base carried forward verbatim:
  SumTree, PrioritizedReplayBuffer, Double DQN in update(), QNetwork (flat,
  non-dueling — v4b does NOT include the v4a Dueling change), freeze-then-finetune,
  column-masked regime protection, Adam momentum reset, rotating eval seeds,
  all hyperparameters.

CHANGES vs v3 in argparse:
  --out_dir default: models/rl_v4b/   (was models/rl_full)
  --mlp / --regime_pkl: results/checkpoints/ (post-restructure path, unchanged from v3)
  --curriculum_p1 / --curriculum_p2: schedule breakpoints (default 1500 / 3000)
  --print_curriculum_only: print the full schedule table and exit, no training

Usage:
    python src\\train_dqn_v4b_curriculum_rs.py --print_curriculum_only
    python src\\train_dqn_v4b_curriculum_rs.py --variant scratch   --seed 42
    python src\\train_dqn_v4b_curriculum_rs.py --variant warmstart --seed 42
    (Stage 2 = 3 seeds [42,123,7] x 2 variants = 6 runs)
"""
...
def print_curriculum_schedule(p1, p2, total_episodes, scenario_order):
    print("\n" + "=" * 72)
    print("  v4b CURRICULUM SCHEDULE  (tier weights at representative episodes)")
    print("=" * 72)
...
def main():
    ...
    ap.add_argument('--out_dir',    default='models/rl_v4b')
