"""
train_mlp.py
Phase 3b — MLP warm-start model (PyTorch).

Same data, same split, same features, same labels as XGBoost.
The MLP's weights will initialize the RL policy network in Phase 5.

Architecture: 22 → 128 → 64 → 1 (sigmoid)
Loss: weighted BCE (consensus_confidence as sample weight)
Class balance: pos_weight = neg_count / pos_count

Usage:
    python src\\train_mlp.py
    python src\\train_mlp.py --epochs 50 --lr 0.001
    python src\\train_mlp.py --batch_size 4096
"""
...
# ─── Must match train_warmstart.py exactly ────────────────────────────────────
...
class NextHopMLP(nn.Module):
    ...
    This exact architecture (input_dim → 128 → 64 → 1) will be reused
    as the policy head in the RL agent. Keeping it simple and small
    ensures the RL phase trains efficiently.
    """
    ...
    def forward(self, x):
        return self.net(x).squeeze(-1)   # raw logits (no sigmoid — BCE with logits)

# ─── Data loading (reuses logic from train_warmstart.py) ──────────────────────
...
def prepare_tensors(df, device, feat_mean=None, feat_std=None):
    """Convert DataFrame to normalized torch tensors. Returns X, y, w, stats."""
    ...
    if feat_mean is None:
        ...
        feat_std  = X.std(axis=0) + 1e-8   # avoid /0
...
# ─── Top-k accuracy (same logic as XGBoost script) ───────────────────────────
...
def train_epoch(model, loader, optimizer, criterion, pos_weight_t):
    model.train()
    total_loss = 0.0
    n = 0
...
def main():
    ...
    print(f"\n  Train: {len(X_tr):>10,} rows  ({n_pos:,}+ / {n_neg:,}−)")
    print(f"  Val:   {len(X_va):>10,} rows")
    print(f"  Test:  {len(X_te):>10,} rows")
    ...
    print(f"\n  Model: {n_params:,} parameters "
          f"({n_params*4/1024:.1f} KB at float32)")
    ...
    model_path = os.path.join(args.out_dir, 'mlp_warmstart.pt')
    torch.save(save_bundle, model_path)
    print(f"\n  Model saved → {model_path}")
    print(f"  (Contains: weights, normalization stats, feature list)")
    print(f"  (Use feat_mean/feat_std to normalize inputs at inference)")
    ...
    if not args.no_plot:
        ...
        axes[0].set_title('MLP — Loss', fontweight='bold')
        axes[0].legend()
        ...
        plt.tight_layout()
        plot_path = os.path.join(args.out_dir, 'mlp_learning_curve.png')
    ...
    print(f"\n{'═'*60}")
    print(f"  DONE — mlp_warmstart.pt ready for RL policy initialization")
    print(f"{'═'*60}\n")
