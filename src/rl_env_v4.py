"""
rl_env_v4.py  — Approach 1 env with v4 REWARD SHAPING (link-quality-weighted progress).

DELTA vs rl_env_full.py:
  EXACTLY ONE behavioural change, inside _compute_reward():
      OLD:  r_prog = 1.0 * (d_before - d_after) / max(d_before, 1.0)
      NEW:  r_prog = progress * (0.5 + 0.5 * link_quality)
  where `progress` is the same normalised distance-reduction term as before and
  `link_quality` is the link_quality attribute of the hop's edge (the same `lq`
  already used by the independent r_lq = 0.3 * lq term). The lq/lt edge lookup is
  simply moved ABOVE the r_prog line so lq is in scope when r_prog is computed.
  Everything else (Packet, graph builder, step/reset, observation, scenarios,
  make_env) is byte-for-byte identical to rl_env_full.py.

  INTENT (deliberate double-count, NOT an accident): progress made over a good
  link is now rewarded more than the same progress made over a marginal link.
  Because (0.5 + 0.5*lq) in [0.5, 1.0], the new r_prog is always <= the old
  r_prog, so this SHRINKS total progress reward and gates it on link quality —
  it does not inflate reward. The lq-sensitivity added to r_prog is 0.5*progress,
  which for typical per-hop progress (~0.1-0.3) is 0.05-0.15, i.e. well below the
  standing r_lq lq-sensitivity of 0.3. See src/preflight_reward_v4.py for the
  mandatory magnitude sanity check that verifies this on real transitions.
...
class FANETRoutingEnv(gym.Env):
    ...
    def _compute_reward(self, pkt, next_hop, d_before, d_after,
        ...
        # v4: lq/lt edge lookup moved ABOVE r_prog so link_quality is in scope
        # for the link-quality-weighted progress term.
        lq = lt = 0.0
        ...
        # v4 REWARD SHAPING — the ONE change vs rl_env_full.py:
        #   old: r_prog = 1.0 * (d_before - d_after) / max(d_before, 1.0)
        #   new: r_prog = progress * (0.5 + 0.5 * link_quality)
        progress = (d_before - d_after) / max(d_before, 1.0)
        r_prog  = progress * (0.5 + 0.5 * lq) if not dropped else 0.0
        r_lq    = 0.3 * lq
