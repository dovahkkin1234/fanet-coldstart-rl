"""
train_dqn_v4a_dueling_rs.py  — v4a: Dueling head + reward shaping.

LINEAGE
-------
Exactly train_dqn_v3_ddqn_per.py (Double DQN + Prioritized Experience Replay)
with two targeted additions:

  1. QNetwork -> DuelingQNetwork                                    [v4a new]
     Shared backbone (Linear(in,128), ReLU, Dropout, Linear(128,64), ReLU)
     + two separate heads: value_head = Linear(64,1), advantage_head = Linear(64,1).
     forward() returns (V, A) SEPARATELY — no Q combination inside forward,
     because mask-aware mean(A) requires the valid-candidate mask, which is an
     env concern, not a network concern.
     Q = V + A - mean_valid(A) is computed in _combine_q(), called by both
     select_action() and update() with the mask.
     Backbone maps exactly to MLP net.0/net.3 for clean warmstart weight transfer:
       shared[0] <- MLP net.0   (weight + bias)
       shared[3] <- MLP net.3   (weight + bias)
     Both heads are ALWAYS randomly initialised (MLP's net.6 is intentionally
     dropped — it was a single output and does not map to two heads).
     Freeze / column-mask protection retargeted to shared[0] / shared[3].

  2. Env import -> rl_env_v4 (link-quality-weighted r_prog)         [v4a new]
     Old: r_prog = 1.0 * progress
     New: r_prog = progress * (0.5 + 0.5 * link_quality)
     Run src/preflight_reward_v4.py first — mandatory pre-flight check.
     NOTE: the check found link_quality saturates near 1.0 in this distance-only
     link model (multiplier >= 0.95 on 100% of captured hops, mean r_prog change
     = 0.3%). Expect the reward-shaping arm to show little effect FOR THIS REASON
     (no interference/multipath), not because the implementation is wrong.

v3 base carried forward verbatim:
  SumTree, PrioritizedReplayBuffer, Double DQN in update(),
  freeze-then-finetune, column-masked regime protection, Adam momentum reset,
  rotating eval seeds, all hyperparameters.

CHANGES vs v3 in argparse:
  --out_dir default: models/rl_v4a/   (was models/rl_full)
  --mlp / --regime_pkl: results/checkpoints/ (post-restructure path)
  --arch flag added to checkpoint: 'dueling'

Usage:
    python src\\train_dqn_v4a_dueling_rs.py --variant scratch   --seed 42
    python src\\train_dqn_v4a_dueling_rs.py --variant warmstart --seed 42
    (Stage 1 full combo = 3 seeds [42,123,7] x 2 variants = 6 runs)
"""
...
# v4a: reward-shaped env (link-quality-weighted progress). Everything else in
# rl_env_v4 is byte-identical to rl_env_full.
...
class DuelingQNetwork(nn.Module):
    ...
    def forward(self, x):
        h = self.shared(x)
        # returns (V, A), each (..., 1); NO Q combination here (needs the mask).
        return self.value_head(h), self.advantage_head(h)


# ─── Prioritized Experience Replay (SumTree) — from train_dqn_v3_ddqn_per.py ──
class SumTree:
    """Binary tree: leaves hold priorities, internals hold sums.
    O(log n) sampling and update. Taken verbatim from train_dqn_v3_ddqn_per.py."""
    def __init__(self, capacity):
        self.capacity = capacity
        self.tree  = np.zeros(2 * capacity)   # size 2*cap; leaf i at i+cap-1
...
class PrioritizedReplayBuffer:
    """PER with SumTree. Taken verbatim from train_dqn_v3_ddqn_per.py.
    alpha controls prioritization strength; beta is the IS-correction exponent
    (annealed from beta_start to 1.0 over training)."""
...
class DQNAgent:
    def __init__(self, input_dim, lr_head=1e-4, lr_backbone=1e-4, gamma=0.95,
        ...
        self.q_net      = DuelingQNetwork(input_dim).to(device)   # v4a
        self.target_net = DuelingQNetwork(input_dim).to(device)   # v4a
        self.target_net.load_state_dict(self.q_net.state_dict())

        # Dropout must never be active for the target net, and greedy eval must
        # be deterministic. Dropout(p) is a no-op in .eval(); gradients still flow.
        self.q_net.eval()
        self.target_net.eval()

        # v4a: backbone = shared[0]+shared[3]; head = value_head+advantage_head.
        ...
        self.freeze_until_episode = 0
        self.protect_input_cols   = None
        self._unfroze_already     = False

    # ─── v4a: mask-aware dueling Q combination ────────────────────────────────
    ...
    def load_warmstart(self, mlp_state_dict):
        ...
        src_w0 = mlp_state_dict['net.0.weight']       # (128, mlp_in)
        src_b0 = mlp_state_dict['net.0.bias']         # (128,)
        src_w3 = mlp_state_dict['net.3.weight']       # (64, 128)
        src_b3 = mlp_state_dict['net.3.bias']         # (64,)
        ...
        tgt_w0 = own['shared.0.weight'].clone()       # (128, input_dim)
        if mlp_in == self.input_dim:
            tgt_w0 = src_w0.clone()
            pad_msg = "EXACTLY"
        else:
            tgt_w0[:, :mlp_in] = src_w0               # copy pretrained cols
    ...
    def protect(self, freeze_episodes, protect_input_cols=None):
        """Freeze pretrained backbone for `freeze_episodes`. protect_input_cols:
        None -> freeze shared[0]+shared[3] entirely (warmstart); int -> freeze
        shared[0].bias + shared[3], keep shared[0].weight trainable EXCEPT the
        first `protect_input_cols` columns (snapshot/restore each update)."""
    ...
    def update(self, batch, episode, is_weights=None):
        """Double DQN + IS-weighted Huber loss. Returns (loss_scalar, td_errors)
        where td_errors is a numpy array of absolute TD errors for PER reprioritisation."""
        if len(batch) == 0:
        ...
        cur_masks = torch.tensor(np.stack([b[0]['mask'] for b in batch]),
                                 dtype=torch.float32, device=self.device)   # v4a: needed for dueling
        ...
        q_all   = self._combine_q(self.q_net, feats, cur_masks)      # (B, MN)
        q_taken = q_all.gather(1, actions.unsqueeze(1)).squeeze(1)

        with torch.no_grad():
            # Double DQN (from v3): online net selects a*, target evaluates.
            # a* = argmax_{a'} Q_online(s',a')  then target = r + gamma*Q_target(s',a*)
            q_next_online = self._combine_q(self.q_net, nxt_feats, nxt_masks)
            q_next_online = q_next_online.masked_fill(nxt_masks == 0, -1e9)
            best_actions  = q_next_online.argmax(dim=1, keepdim=True)
            q_next_target = self._combine_q(self.target_net, nxt_feats, nxt_masks)
            q_next_sel    = q_next_target.gather(1, best_actions).squeeze(1)
            target = rewards + self.gamma * q_next_sel * (1.0 - dones)

        td_errors = (q_taken.detach() - target).abs().cpu().numpy()   # for PER reprioritisation
        per_sample_loss = nn.functional.smooth_l1_loss(q_taken, target, reduction='none')
        loss = (is_w * per_sample_loss).mean() if is_w is not None else per_sample_loss.mean()
...
def train_variant(variant_name, mlp_bundle, regime_bundle, args, device):
    print(f"\n{'='*60}\n  Training v4a: {variant_name}  (seed={args.seed})\n{'='*60}")
...
def plot_eval_curves(all_histories, out_path, scenario_names):
    ...
    plt.suptitle('v4a Cold-Start Learning Curves (Dueling + reward shaping, batch greedy eval)',
...
def main():
    ...
    # Spec path note: new scripts default to results/checkpoints/, NOT models/.
    ...
    ap.add_argument('--out_dir',    default='models/rl_v4a')
    ...
    ap.add_argument('--lr_backbone', type=float, default=1e-5,
                    help='Pretrained backbone LR after unfreezing (warmstart/regime). '
                         'Scratch uses lr_head uniformly.')
    ...
    # Reproducibility: seed torch (weight init) AND global np.random (PER sampling
    # + epsilon-greedy) at the very top, before any nn.Module is constructed.
    ...
    if len(all_metrics) > 1:
        print(f"\n{'='*70}\n  v4a COLD-START COMPARISON (training-scenario eval)\n{'='*70}")
